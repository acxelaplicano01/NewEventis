<div class="bg-white dark:bg-stone-800 rounded-lg shadow-md p-6">
	<div class="mb-6">
		<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
			...
			<div class="flex flex-col sm:flex-row w-full sm:w-auto space-y-3 sm:space-y-0 sm:space-x-2">
				<div class="relative w-full sm:w-auto">
					<x-input wire:model.live="search" type="text" placeholder="Buscar modalidades..."
		...
		['value' => '100', 'text' => '100 por página'],
				...
				@can('mantenimiento.modalidades.crear')
	...
	<x-table
		...
		empty-message="No hay modalidades registradas"
		class="mt-6"
	>
		<x-slot name="desktop">
			@forelse($modalidades as $mod)
				<tr class="hover:bg-stone-50 dark:hover:bg-stone-700 transition-colors">
					<td class="px-6 py-4 whitespace-nowrap text-stone-900 dark:text-stone-300">{{ $mod->id }}</td>
					<td class="px-6 py-4 text-stone-900 dark:text-stone-300">{{ $mod->modalidad }}</td>
					<td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
						<div class="flex justify-end space-x-2">
							<button wire:click="edit({{ $mod->id }})"
                            ...
                            <button wire:click="confirmDelete({{ $mod->id }})"
				...
				<div
					...
					<div class="flex justify-between items-center">
						<div>
							...
							<h3 class="font-semibold text-stone-900 dark:text-stone-200 text-lg mt-2">{{ $mod->modalidad }}
							</h3>
						</div>
	...
	<x-elegant-delete-modal wire:model="confirmingDelete" title="Eliminar Modalidad"
		message="¿Estás seguro de que deseas eliminar esta modalidad?" entityName="{{ $nombreAEliminar }}"
