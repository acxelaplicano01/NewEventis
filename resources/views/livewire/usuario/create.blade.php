    ...
    <x-dialog-modal wire:model="isOpen" maxWidth="md">
        <x-slot name="title">
            <div class="flex justify-between items-center">
                <h3 class="text-lg font-semibold text-stone-900 dark:text-white">{{ $isEditing ? 'Editar' : 'Nuevo' }}
                    Usuario</h3>
        ...
        <x-slot name="content">
            <form id="userForm" class="space-y-4">
                ...
                <div>
                    ...
                    <div
                        class="mt-2 grid grid-cols-1 gap-3 max-h-60 overflow-y-auto p-2 border border-stone-200 dark:border-stone-700 rounded-md">
                        @forelse($roles ?? [] as $role)
                            @if(is_object($role))
                                <div class="py-1">
                                    <x-toggle wire:model="selectedRoles" value="{{ $role->id }}">
                                        {{ $role->name }}
                                    </x-toggle>
                                </div>
                            @endif
