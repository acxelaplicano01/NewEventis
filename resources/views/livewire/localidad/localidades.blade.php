<div class="bg-white dark:bg-stone-800 rounded-lg shadow-md p-6">
	<div class="mb-6">
		<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
			<h2 class="text-xl font-semibold text-stone-800 dark:text-stone-200">
				{{ __('Administración de Localidades') }}
			</h2>

			<div class="flex flex-col sm:flex-row w-full sm:w-auto space-y-3 sm:space-y-0 sm:space-x-2">
				<div class="relative w-full sm:w-auto">
					<x-input wire:model.live="search" type="text" placeholder="Buscar localidades..."
		...
		['value' => '100', 'text' => '100 por página'],
				...
				@can('mantenimiento.localidades.crear')
	...
	<x-table
		...
		empty-message="No hay localidades registradas"
		class="mt-6"
	>
		<x-slot name="desktop">
			@forelse($localidades as $loc)
				<tr class="hover:bg-stone-50 dark:hover:bg-stone-700 transition-colors">
					<td class="px-6 py-4 whitespace-nowrap text-stone-900 dark:text-stone-300">{{ $loc->id }}</td>
					<td class="px-6 py-4 text-stone-900 dark:text-stone-300">{{ $loc->localidad }}</td>
				...
				<div
					...
					<div class="flex justify-between items-center">
						<div>
							...
							<h3 class="font-semibold text-stone-900 dark:text-stone-200 text-lg mt-2">{{ $loc->localidad }}
							</h3>
						</div>
	...
	<x-elegant-delete-modal wire:model="confirmingDelete" title="Eliminar Localidad"
		message="¿Estás seguro de que deseas eliminar esta localidad?" entityName="{{ $nombreAEliminar }}"
