<x-action-section>
    <x-slot name="title">
        <span class="text-stone-900 dark:text-stone-100">{{ __('Autenticación de Dos Factores') }}</span>
    </x-slot>

    <x-slot name="description">
        <span class="text-stone-600 dark:text-stone-400">{{ __('Añade seguridad adicional a tu cuenta utilizando la autenticación de dos factores.') }}</span>
    </x-slot>
    ...
    <x-slot name="content">
        ...
        @if ($this->enabled)
            @if ($showingQrCode)
                ...
                @if ($showingConfirmation)
                    <div class="mt-4">
                        <x-label for="code" value="{{ __('Código') }}" class="dark:text-stone-300" />

                        <x-input id="code" type="text" name="code" class="block mt-1 w-1/2 dark:bg-stone-800 dark:text-stone-200 dark:border-stone-700" inputmode="numeric" autofocus autocomplete="one-time-code"
                            wire:model="code"
            ...
            @if ($showingRecoveryCodes)
                ...
                <div class="grid gap-1 max-w-xl mt-4 px-4 py-4 font-mono text-sm bg-stone-100 dark:bg-stone-800 dark:text-stone-200 rounded-lg">
        ...
        <div class="mt-5">
            @if (! $this->enabled)
                <x-confirms-password wire:then="enableTwoFactorAuthentication">
                    <x-button type="button" wire:loading.attr="disabled" class="dark:bg-stone-800 dark:border-stone-700 dark:text-white dark:hover:bg-stone-700">
                        {{ __('Activar') }}
                    </x-button>
                </x-confirms-password>
            @else
                @if ($showingRecoveryCodes)
                    <x-confirms-password wire:then="regenerateRecoveryCodes">
                        <x-secondary-button class="me-3 dark:border-stone-700 dark:text-stone-300 dark:hover:bg-stone-700">
                            {{ __('Regenerar Códigos de Recuperación') }}
                        </x-secondary-button>
                    </x-confirms-password>
                @elseif ($showingConfirmation)
                    <x-confirms-password wire:then="confirmTwoFactorAuthentication">
                        <x-button type="button" class="me-3 dark:bg-stone-800 dark:border-stone-700 dark:text-white dark:hover:bg-stone-700" wire:loading.attr="disabled">
                            {{ __('Confirmar') }}
                        </x-button>
                    </x-confirms-password>
                @else
                ...
                @if ($showingConfirmation)
                    <x-confirms-password wire:then="disableTwoFactorAuthentication">
                        <x-secondary-button wire:loading.attr="disabled" class="dark:border-stone-700 dark:text-stone-300 dark:hover:bg-stone-700">
                            {{ __('Cancelar') }}
                        </x-secondary-button>
                    </x-confirms-password>
                @else
